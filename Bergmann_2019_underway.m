...
[ ~, p ]=sort(Velp);
TSG_sea_temp=[]; TSG_date=[];TSG_time=[];
...
%there are no dashes for time add these for clarity!
Date_char=num2str(TSG_date); %convert to char for concatination
Date_cell={};
%add first bracket
for K=1:length(Date_char);
        A= strtrim(regexprep(Date_char(K,:), '.{4}', '$0/','once'));
...
%no colons for time again add for clarity
Time_char=num2str(TSG_time,'%06.f'); %convert the time into a format with leading 0's
Time_cell={};
%add first bracket
for K=1:length(TSG_time);
        A= strtrim(regexprep(Time_char(K,:), '.{2}', '$0:','once'));
        Time_cell(K,:)=cellstr(A);
end
%Add second bracket
...
Dt_Cat=strcat(Date_cell2,{' '}, Time_char2);
Dt_TSG=datenum(Dt_Cat,'yyyy/mm/dd HH:MM:SS');
...
Berg19_co2_CO2umm=CO2_ppm./(Cell_p*0.00986923);
Berg19_co2_dt=Dt;
...
Berg19_co2_Sea_temp=nan(length(Berg19_co2_TSG_t),1);%bad
   ...
   Berg19_co2_TSG_s=Berg19_co2_TSG_s+0.1;
    ...
    Berg19_co2_LicorpCO2= Berg19_co2_CO2umm_cal.*(Berg19_co2_Pres_atm - Vapour_pressure_atm);%Correction for water vapour pressure as described in (dickson(2007)-section 8.5.3 TO GET PCO2
...
clearvars amsr_day l molality osmotic_coef temp_mod vapor_0sal_kPa
